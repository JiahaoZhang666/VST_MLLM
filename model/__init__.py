...
from .videorefer_qwen2 import VideoSpatioTemporalQwen2ForCausalLM, VideoSpatioTemporalQwen2Config


VLLMs = {
    "videospatiotemporal": VideoSpatioTemporalQwen2ForCausalLM,
    "videospatiotemporal_qwen2": VideoSpatioTemporalQwen2ForCausalLM,
}

VLLMConfigs = {
    "videospatiotemporal": VideoSpatioTemporalQwen2Config,
    "videospatiotemporal_qwen2": VideoSpatioTemporalQwen2Config,
}
...
def load_pretrained_model(model_path, model_base, model_name, load_8bit=False, load_4bit=False, device_map="auto", device="cuda", use_flash_attn=False, **kwargs):
    ...
    if model_base is not None or is_pretraining:
        ...
        print('Loading VideoSpatioTemporal from base model...')
        ...
        if model_type in ['videospatiotemporal', 'videospatiotemporal_qwen2']:
            model = VideoSpatioTemporalQwen2ForCausalLM.from_pretrained(model_base, low_cpu_mem_usage=True, config=config, **kwargs)
        else:
            raise NotImplementedError

        mm_projector_weights = load_mm_projector(model_path, token=token)
        model.load_state_dict(mm_projector_weights, strict=False)
    elif 'videospatiotemporal' in model_type:
        # NOTE: SFT model loading
        tokenizer = AutoTokenizer.from_pretrained(model_path, use_fast=False, token=token)

        if model_type in ['videospatiotemporal', 'videospatiotemporal_qwen2']:
            model = VideoSpatioTemporalQwen2ForCausalLM.from_pretrained(model_path, low_cpu_mem_usage=True, config=config, **kwargs)
        else:
            raise NotImplementedError
    else:
    ...
    if "videospatiotemporal" in model_type:
        vision_tower = model.get_vision_tower()
        if not vision_tower.is_loaded:
            vision_tower.load_model()
        vision_tower.to(device=device, dtype=torch.float16)
        # NOTE: VideoSpatioTemporal adopts the same processor for processing image and video.
